...
builder.Services.AddAuthorization(
    x => {
       ...
       p => p.AddRequirements(new AdminAuthRequirement(config["ApiKey"]!)));
        ...
        x.AddPolicy(AuthConstants.TrustedMemberPolicyName, policy => policy.RequireAssertion(ctx => 
        ctx.User.HasClaim(m => m is { Type: AuthConstants.AdminUserClaimName, Value: "true"} ||
        ctx.User.HasClaim(m => m is { Type: AuthConstants.TrustedMemberPolicyName, Value: "true" })
        ))
        );
    });
...
//app.UseCors(); //add this before caching
//app.UseResponseCaching(); // this is client specific caching- the client will decide if it wants to cache the response
app.UseOutputCache(); // only 200 is cached and get, head requests are cached
